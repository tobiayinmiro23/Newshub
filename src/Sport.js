...
const Sport = () => {
    ...
    const handleSearch=(e)=>{
        ...
        setsearching(true)
        setresult([])
        fetch(`https://newsapi.org/v2/top-headlines?country=${country}&category=sport&sortBy=relevancy&page=${page}&apiKey=${process.env.API_KEY}`)
            .then(res=>res.json())
            .then(res=>{
                if(res?.status !== 'ok' ){
                    alert(res?.message)
        ...
        return (
            <div className='Business'>
                <h1 style={{borderBottom:'1px solid #d5d4d4',padding:"0.8rem",textAlign:"center"}}>Sport</h1>
