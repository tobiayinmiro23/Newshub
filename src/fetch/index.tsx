...
export function Category({ setloading, setdataReady, setresults, setnoResult, seterror, country, setpage, category }: categoryI) {
    ...
    const options = {
        method: 'GET',
        url: `https://newsdata.io/api/1/news?apikey=${apiKey}&category=${category}&country=${country}`
    };
    axios.request(options).then(res => {
        setloading(false)
        if (res?.data.status !== 'success') seterror(true)
...
export function LoadMore({ setloadMore, results, country, setpage, page }: loadMoreI) {
    ...
    const options = {
        method: 'GET',
        url: `https://newsdata.io/api/1/news?apikey=${apiKey}&country=${country}&page=${page}`
    };
    axios.request(options).then(res => {
        setloadMore(false)
...
export function LoadMoreWithCategory({ setloadMore, results, country, setpage, page, category }: loadMoreWithCategoryI) {
    ...
    const options = {
        method: 'GET',
        url: `https://newsdata.io/api/1/news?apikey=${apiKey}&category=${category}&country=${country}&page=${page}`
    };
    axios.request(options).then(res => {
        setloadMore(false)
        console.log(res)
...
export function Search({ setloading, setdataReady, setresults, setnoResult, seterror, search, setpage }: searchI) {
    ...
    const options = {
        method: 'GET',
        url: `https://newsdata.io/api/1/news?apikey=${apiKey}&q=${search}&`
    };
    axios.request(options).then(res => {
        setloading(false)
        console.log(res)
